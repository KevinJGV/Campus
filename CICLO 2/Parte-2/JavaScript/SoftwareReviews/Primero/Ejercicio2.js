let oldcampers = Number(prompt("Campers antiguos:")) ; let newcampers = Number(prompt("Campers nuevos:")) ; let pc = Number(prompt("PC\'s de la empresa:")); let totalcampers = oldcampers + newcampers; let jornadas = Math.ceil(totalcampers / pc);












if (pc < totalcampers) {
    alert('Hay más Campers que pcs disponibles en CampusLands, es necesario seccionar por jornadas para dar una solución a la diferencia.');
    let newflag = newcampers; let oldflag = oldcampers; let pcflag = pc; let jornadasflag = jornadas
    if ()


}